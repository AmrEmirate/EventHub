    ...
    <>
        ...
        <CardHeader>
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <CardDescription>
                Ini adalah daftar tiket event yang telah Anda beli.
              </CardDescription>
            </div>
            <Button asChild variant="outline" className="w-full sm:w-auto">
              <Link href="/profile/transactions">
                <History className="mr-2 h-4 w-4" /> Lihat Semua Transaksi
              </Link>
            </Button>
          </div>
        </CardHeader>
        <CardContent className="space-y-4">
          {transactions.filter((t) => t.status === "COMPLETED").length > 0 ? (
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {new Date(trx.event.startDate).toLocaleDateString(
                        "id-ID",
                        {
                          weekday: "long",
                          day: "numeric",
                  ...
                  <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto shrink-0">
                    <Button
                      ...
                      onClick={() => handleOpenReviewModal(trx)}
                    >
            ...
            <div className="text-center text-muted-foreground py-10">
              ...
              <p className="text-sm">
                Beli tiket event untuk melihat e-tiket Anda di sini.
              </p>
            </div>
          )}
        </CardContent>
      ...
      <Dialog open={isReviewModalOpen} onOpenChange={setIsReviewModalOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Beri Ulasan untuk {selectedTransaction?.event.name}
            </DialogTitle>
            <DialogDescription>
              Bagikan pengalaman Anda. Ulasan Anda akan sangat membantu pengguna
              lain.
            </DialogDescription>
          </DialogHeader>
          {selectedTransaction && (
